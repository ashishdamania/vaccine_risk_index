...
final_rank_with_country_classification %>%
  pivot_longer(.,cols=c(ML1,ML2,ML3,ML4,ML5), names_to="Factors") %>%
  ggplot(., aes(x=Factors, y=value, color=WHO_REGION_FULL)) +
  geom_boxplot() +
  facet_grid(~WHO_REGION_FULL)+
  theme_bw()+
  theme(axis.text.x = element_text(angle = 90))+
  ylab("Factor scores") +
  theme(legend.position='none')
ggsave("Factor_scores_by_WHO_region_classification.pdf", width = 11, dpi=300,path = "../Figures")
```
